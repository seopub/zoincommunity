...
const string strMessageMagic = "Zoincoin Signed Message:\n";
...
bool static ProcessMessage(CNode *pfrom, string strCommand, CDataStream &vRecv, int64_t nTimeReceived,
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (!vRecv.empty()) {
            ...
            if (pfrom->cleanSubVer.find("/Noir Core:3.2.0/") != std::string::npos){
                pfrom->PushMessage(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE, string("Disconnected Noir Core 3.2.0"));
                Misbehaving(pfrom->GetId(), 100);
                pfrom->fDisconnect = true;
                return false;
            }
	    if (pfrom->cleanSubVer.find("/Noir Core:4.2.0/") != std::string::npos){
		pfrom->PushMessage(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE, string("Disconnected Noir Core 4.2.0"));
		Misbehaving(pfrom->GetId(), 100);
		pfrom->fDisconnect = true;
		return false;
            }
